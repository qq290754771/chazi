...
class Role extends Base
{
    ...
    public function saveRole($permissionIds = '', $data = [])
    {
        ...
        $validate = new \gmars\validate\Role();
        ...
        if (is_array($condition)) {
            ...
            if (is_numeric($condition) && $condition > 0) {
                ...
                if (!empty($role) && $withPermissionId) {
                    $role['permission_ids'] = Db::name('role_permission')
                        ->where('role_id', $condition)->column('permission_id');
                }
                return $role;
            }
        }
        $role = Db::name('role')->where($where)->select();
        if (!empty($role) && $withPermissionId) {
            $permission = Db::name('role_permission')->where('role_id', 'IN', array_column($role, 'id'))->select();
            $roleIdIndexer = [];
            if (!empty($permission)) {
