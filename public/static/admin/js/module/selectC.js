...
layui.define(["jquery", "laydate", "form"], function (exports) {
  ...
  obj.prototype.init = function () {
    ...
    html +=
      '<form id="selectC-form" class="layui-form"><div class="layui-form-item">';
    for (var i = 0; i < n; i++) {
      html +=
      ...
      html += "</dl></div></div>";
      html +=
        '<div class="layui-input-inline" id="inputBox' +
        i +
        '">' +
    ...
    if (config.reset) {
      html +=
        '<button type="button" class="layui-btn layui-btn-primary" id="selectC-reset">清空</button>';
    }
    html += "</div></div></form>";
    elem.append(html);
    /**搜索按钮点击事件**/
      ...
      if (n == o[l].name) {
        if (o[l].type == "input") {
          ...
          for (var k = 0; k < data.length; k++) {
            html +=
              '<option value="' +
              data[k][field["valueField"]] +
              '">' +
              data[k][field["nameField"]] +
              "</option>";
          }
          html += "</select>";
      ...
      .on("click", function (e) {
        ...
        if (l.is(".layui-form-selected")) {
          ...
          $(document).on("click", function (e) {
            if (
              e.target.id !== l.attr("id") &&
              e.target.className !== "layui-input layui-unselect"
            ) {
              l.removeClass("layui-form-selected");
              $(document).off("click");
            }
  ...
  obj.prototype.getSelectData = function (url, optionNum) {
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.code == 0) {
            that.config.options[optionNum].data =
