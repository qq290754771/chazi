...
{
    $tempfiles = dir_list(APP_PATH . '../templete/default/', 'html');
    // dump($tempfiles);
 ...
 * 获取文章内容html中第一张图片地址
 */
function get_html_first_imgurl($str)
{
...
function str_cut($sourcestr, $cutlength, $suffix = '')
{
    ...
    $i = 0;
    $n = 0;
    $sourcestr = DeleteHtml($sourcestr);
...
{
    if ($field) {
        $name = db('permission')->where('id', $id)->value($field);
        return $name;
    } else {
        $name = db('permission')->where('id', $id)->value('name');
        return $name;
    }
}
 ...
 * @return [type]       [description]
 */
function subtree($data, $pid = 0, $role_id = null)
{
    if ($role_id) {
        $list = db('role_permission')->field('permission_id')->where('role_id', $role_id)->select();
        $roleid = array_column($list, 'permission_id');
    }
    // dump($data);
    foreach ($data as $key => $vo) {
        if (in_array($vo['id'], $roleid)) {
            $res[$vo['id']] = $vo;
            // $res[$vo['id']]['href'] = url('admin/'.$vo['href']);
            $res[$vo['id']]['children'] = [];
            $res[$vo['id']]['spread'] = true;
        }
    }
    ...
    foreach ($res as $key => $vo) {
        if ($vo['pid'] != $pid) {
            // dump($res[$vo['pid']]['status']);
            if ($res[$vo['pid']]['status'] == 1 && $vo['pid'] != 0) {
                $res[$vo['pid']]['children'][] = &$res[$key];
                $res[$vo['pid']]['spread'] = true;
            }
        }
    }
    // dump($res);
...
function subtreeAuth($data, $pid = 0, $id)
{

    $list = db('role_permission')->field('permission_id')->where('role_id', $id)->select();
    $list = array_column($list, 'permission_id');
    $res = [];
    foreach ($data as $key => $vo) {

        $res[$vo['id']]['id'] = $vo['id'];

        $res[$vo['id']]['title'] = $vo['name'];
        $res[$vo['id']]['pid'] = $vo['pid'];
    ...
    // 查询子孙
    foreach ($res as $key => $vo) {
        if (in_array($vo['id'], $list)) {
            $res[$vo['id']]['checked'] = true;
            $res[$vo['pid']]['checked'] = false;
...
function show($status, $message, $data = [], $httpCode = 200)
{
    ...
    return json($data, $httpCode)->send();
    exit();
}

function getCategorys($catid,$field){
    $name = \think\Db::name('permission')->where('id',$catid)->value($field);
    return $name;
}

function getField($catid,$id,$field){
    $table = \think\Db::name('permission')->where('id',$catid)->value('module');
    $name = \think\Db::name($table)->where('id',$id)->value($field);
    return $name;
}
